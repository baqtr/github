...
heroku_postgres_uri = os.getenv("DATABASE_URL","postgres://u7sp4pi4bkcli5:p8084ef55d7306694913f43fe18ae8f1e24bf9d4c33b1bdae2e9d49737ea39976@ec2-18-210-84-56.compute-1.amazonaws.com:5432/dbdstma1phbk1e")
# إنشاء كائن البوت
...
def get_db_connection():
    return psycopg2.connect(heroku_postgres_uri)
...
def add_account_to_db(user_id, api_key):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("""
        INSERT INTO user_accounts (user_id, api_key) VALUES (%s, %s)
    """, (user_id, api_key))
    conn.commit()
    cursor.close()
    conn.close()

# دالة لجلب حسابات المستخدم من قاعدة البيانات
...
def add_event_to_db(user_id, event_text):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("""
        INSERT INTO events (user_id, event_text) VALUES (%s, %s)
    """, (user_id, event_text))
    conn.commit()
    cursor.close()
    conn.close()

# دالة لجلب الأحداث من قاعدة البيانات
def get_events():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT event_text FROM events ORDER BY event_time DESC")
