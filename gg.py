...
def load_accounts():
    ...
    user_accounts = {row['user_id']: row['accounts'] for row in rows}
    cursor.close()
    conn.close()
...
def save_accounts():
    ...
    for user_id, accounts in user_accounts.items():
        cursor.execute('''
            INSERT INTO user_accounts (user_id, accounts)
            VALUES (%s, %s)
            ON CONFLICT (user_id) DO UPDATE
            SET accounts = EXCLUDED.accounts;
        ''', (user_id, json.dumps(accounts)))
    conn.commit()
    cursor.close()
    conn.close()
...
def save_event(event):
    ...
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO events (event) VALUES (%s);
    ''', (event,))
    conn.commit()
    cursor.close()
    conn.close()
...
def load_settings():
    ...
    conn = get_db_connection()
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    cursor.execute('SELECT * FROM settings WHERE key = %s;', ('safe_mode',))
    row = cursor.fetchone()
    if row:
        safe_mode = row['value']['enabled']
    cursor.close()
    conn.close()
...
def save_settings():
    ...
    ''', ('safe_mode', json.dumps({'enabled': safe_mode})))
    conn.commit()
    cursor.close()
    conn.close()
